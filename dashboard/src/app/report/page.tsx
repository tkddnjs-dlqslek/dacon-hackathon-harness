...
export default async function ReportPage() {
  ...
  return (
    <div className="space-y-6 print:bg-white print:text-black">
      {/* 헤더 */}
      <header className="rounded-lg border border-gray-200 bg-gray-50 p-6">
      ...
      {/* 시장 레짐 */}
      <section className="rounded-lg border-2 border-gray-200 bg-gray-50 p-6">
        <h2 className="mb-3 text-sm font-semibold uppercase text-gray-500">시장 레짐 판단</h2>
        <p className={`text-3xl font-bold ${regimeColor}`}>{regime}</p>
        <p className="mt-2 text-sm text-gray-500">{regimeDesc}</p>
        <div className="mt-3 grid grid-cols-3 gap-3 text-xs">
          ...
          <div>
            ...
            <p className={`font-mono text-lg ${tnxChange >= 0 ? "text-red-400" : "text-gray-600"}`}>
          ...
          {classStats.map((c, i) => (
            <div key={c.type} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className={`h-5 rounded ${c.avgRet >= 0 ? "bg-green-500" : "bg-red-500"}`}
              ...
              {c.best && (
                <span className="w-32 text-xs text-gray-500">
                  베스트: <span className="font-mono text-gray-600">{c.best.ticker}</span> ({(c.best.ret * 100).toFixed(0)}%)
                </span>
              )}
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <section className="rounded-lg border border-green-900/40 bg-gray-50 p-4">
          ...
          <div className="space-y-2">
            {topGainers.map((x, i) => (
              <div key={x.asset.ticker} className="flex items-center justify-between text-sm">
                ...
                <span className="text-xs text-gray-500 mr-2">{x.asset.name.slice(0, 20)}</span>
                <span className="font-mono text-green-400">+{((x.m?.ret ?? 0) * 100).toFixed(1)}%</span>
              </div>
            ))}
          </div>
        </section>
        <section className="rounded-lg border border-red-900/40 bg-gray-50 p-4">
      ...
      {/* 위험·안전 자산 */}
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <section className="rounded-lg border border-yellow-900/40 bg-gray-50 p-4">
          <h2 className="mb-3 text-sm font-semibold text-red-400">⚠ 변동성 최고</h2>
          <div className="space-y-2">
            {highVol.map((x) => (
              <div key={x.asset.ticker} className="flex items-center justify-between text-sm">
                <span className="font-mono font-bold">{x.asset.ticker}</span>
                <span className="text-xs text-gray-500 flex-1 ml-2">{x.asset.name.slice(0, 25)}</span>
                <span className="font-mono text-red-400">σ {((x.m?.vol ?? 0) * 100).toFixed(0)}%</span>
              </div>
            ))}
          </div>
        </section>
        <section className="rounded-lg border border-cyan-900/40 bg-gray-50 p-4">
          <h2 className="mb-3 text-sm font-semibold text-gray-900">🛡 변동성 최저 (안정)</h2>
          <div className="space-y-2">
            {safeAssets.map((x) => (
              <div key={x.asset.ticker} className="flex items-center justify-between text-sm">
                <span className="font-mono font-bold">{x.asset.ticker}</span>
                <span className="text-xs text-gray-500 flex-1 ml-2">{x.asset.name.slice(0, 25)}</span>
                <span className="font-mono text-gray-900">σ {((x.m?.vol ?? 0) * 100).toFixed(0)}%</span>
              </div>
            ))}
          </div>
        </section>
          ...
          {corrs.map((c, i) => (
            <div key={i} className="rounded border border-gray-200 bg-white p-3">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-mono text-lg ${Math.abs(c.value) > 0.5 ? "text-gray-900" : "text-gray-600"}`}>
                  {c.value.toFixed(2)}
                </span>
              </div>
